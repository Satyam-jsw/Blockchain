...
contract optimized_healthCare {
    ...
    constructor() public {
        ...
        owner = 0x6e66D10a6B4f818A03B80a66Fbc8ffDCD140378a;
    }
    ...
    function grantAccessToDoctor(
        ...
        emit grantDoctorAccess(
            msg.sender,
            "Granted access to doctor",
            d.name,
            doctor_id
        );
    }
    ...
    function revokeAccessFromDoctor(
        ...
        emit revokeDoctorAccess(
            msg.sender,
            "Revoked access of doctor",
            d.name,
            doctor_id
        );
    }
    ...
    function getPatientInfoForDoctor(
        ...
        public
        view
        returns (string memory, string memory, address, files[] memory)
    {
    ...
    function hospitalGrantAccess(
        address _user,
        address _patient
    ) public checkPatient(_patient) {
    ...
    function regInsuranceComp(
        address _id,
        string memory _name
    ) public onlyOwner {
        insuranceComp memory i = insuranceCompanies[_id];
        require(!(i.id > address(0x0)));
        insuranceCompanies[_id] = insuranceComp({
            id: _id,
            name: _name,
            regPatients: new address[](0)
        });
    }

    function getInsuranceCompInfo()
        public
        view
        returns (address, string memory, address[] memory)
    {
        insuranceComp memory i = insuranceCompanies[msg.sender];
        return (i.id, i.name, i.regPatients);
    }

    function addPatientToInsuranceComp(
        address _insuranceComp,
        address _pat
    ) public {
    ...
    function addPatientAdhaarInfo(
        ...
        require(a.adhaar_number == 0);
        patient_adhaar_info[_pat] = adhaar({
            id: _pat,
            pincode: _pincode,
            name: _name,
            DOB: _DOB,
